 ...
 */
func calcluateArea(x:Double , y :Double) ->Double?
...
print(calcluateArea(x: 2.0, y: 3.0)!)
print(calcluateArea(x: -2.0, y: 3.0))
...
{
    guard let name = firstNameTextField.text ,let last = lastNameTextField.text   , let age  = ageTextField.text  else
    {
        return nil
    }
    return User(firstName: name, lastName: last, age: age)
}
...
var user = createUser()
if let ans = user{
    print("First Name is \(ans.firstName) \(ans.lastName) of age \(ans.age)")
}
