...
if let value = dict.updateValue(29, forKey: "February")
{
    print(dict)
}

//:  Use if-let syntax to retrieve the number of days under "January." If the value is there, print "January has 31 days", where 31 is the value retrieved from the dictionary.
if dict.keys.contains("January")
{
    if let val = dict["January"]
    {
        print("January has \(val) days")
    }
}

//:  Given the following arrays, create a new [String : [String]] dictionary. `shapesArray` should use the key "Shapes" and `colorsArray` should use the key "Colors." Print the resulting dictionary.
var shapesArray:[String] = ["Circle","Triangle","Rectangle"]
var colorsArray = ["Red","Blue","Black"]
var allArray = ["Shapes":shapesArray,"Color":colorsArray]
print(allArray)
...
if let color = allArray["Color"]
{
    let index = color.count
    print(color[index-1])
}
